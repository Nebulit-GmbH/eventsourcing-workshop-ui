...
import { useConfirmedAccounts, useConfirmationMailsToSend, useAccountActions } from '@/hooks/useAccount';
import { Users, Mail, CheckCircle, Send } from 'lucide-react';
import { toast } from 'sonner';

export default function ConfirmedAccounts() {
  const { data: confirmedAccounts, isLoading: loadingConfirmed } = useConfirmedAccounts();
  const { data: pendingNotifications, isLoading: loadingPending } = useConfirmationMailsToSend();
  const { notifyCustomer } = useAccountActions();
  const [sendingTo, setSendingTo] = useState<string | null>(null);

  const handleSendNotification = async (userId: string, email: string, name: string) => {
    setSendingTo(userId);
    try {
      await notifyCustomer(userId, email, name);
  ...
  const isLoading = loadingConfirmed || loadingPending;
          ...
          <>
                  ...
                  {pendingNotifications.map((item) => (
                    <Card key={item.userId}>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <Button
                          size="sm"
                          onClick={() => handleSendNotification(item.userId, item.email, item.name)}
            ...
            <div className="space-y-4">
              <h2 className="font-serif text-xl font-medium flex items-center gap-2">
                ...
                Confirmed Accounts
                <Badge variant="secondary">{confirmedAccounts?.length || 0}</Badge>
              </h2>
